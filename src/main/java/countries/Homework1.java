...
public class Homework1 {
    ...
    public void streamPipeline5() {
        countries.stream().sorted(Comparator.comparingLong(country -> country.getPopulation())).map(country -> country.getName())
    ...
    public void streamPipeline7() {
        countries.stream().sorted(Comparator.comparingInt(country -> country.getTimezones().size()))
                .map(country -> country.getName()).forEach(System.out::println);
    }
...
ed by the language code "es").
    ...
    public long streamPipeline9() {
        return countries.stream().filter(country -> !(country.getTranslations().containsValue("es"))).count();
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> country.getArea() != null && country.getArea().compareTo(BigDecimal.ONE) == -1)
                .map(country -> country.getName()).forEach(System.out::println);
    }
    ...
    public void streamPipeline16() {
        countries.stream().filter(country -> country.getRegion().equals(Region.EUROPE) || country.getRegion().equals(Region.ASIA))
                .flatMap(country -> country.getTimezones().stream()).distinct().forEach(System.out::println);
    }
